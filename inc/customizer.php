	...
	function cpschool_theme_customizer() {
		...
		Kirki::add_panel(
			...
			// SECTION - Site Identity (WP)
				...
				// SETTING - Custom Secondary Logo Url
				Kirki::add_field(
					'cpschool',
					array(
						'type'            => 'link',
				...
				// SETTING - Custom Logo Url
				Kirki::add_field(
					'cpschool',
					array(
						'type'        => 'link',
						'settings'    => 'custom_home_url',
						'label'       => esc_attr__( 'Custom Logo / Site Title Link', 'cpschool' ),
						'description' => esc_html__( 'Set to customize logo / site title link.', 'cpschool' ),
						'section'     => 'title_tagline',
						'transport'   => 'refresh',
					)
				);
		...
		Kirki::add_section(
			...
			Kirki::add_field(
				...
				array(
					...
					'label'           => esc_html__( 'Logo Image Size (px)', 'cpschool' ),
		...
		Kirki::add_section(
			...
			array(
				...
				'description' => esc_html__( 'Settings for secondary header, place for less important navigation elements. Please keep in mind that it will be visible only when it has at least one menu attached or secondary logo is set in "General" > "Site Identity".', 'cpschool' ),
			)
		);
			...
			// SETTING - Move Under Primary
			Kirki::add_field(
				'cpschool',
				array(
					'type'      => 'toggle',
					'settings'  => 'header_secondary_under_primary',
					'label'     => __( 'Move Under Main Header', 'cpschool' ),
					'section'   => 'header_secondary',
					'transport' => 'refresh',
					'default'   => false,
				)
			...
			// SETTING - Secondary Logo Image Size
			// TODO overwrite default with actual logo image width on the fly
			Kirki::add_field(
				'cpschool',
				array(
					'type'            => 'slider',
					'settings'        => 'header_secondary_logo_image_width',
					'label'           => esc_html__( 'Secondary Logo Image Size (px)', 'cpschool' ),
					'description'     => esc_html__( 'Width of the seconadry logo image.', 'cpschool' ),
					'section'         => 'header_secondary',
					'default'         => '200',
					'choices'         => array(
						'min'  => 100,
					...
					'output'          => array(
						array(
							'element'  => ':root',
							'property' => '--header-secondary-logo-image-width',
							'units'    => 'px',
						),
					),
				)
		...
		Kirki::add_panel(
			...
			Kirki::add_section(
				'search',
				array(
					'title'       => esc_html__( 'Search', 'cpschool' ),
					'description' => esc_html__( 'Settings related to pages with search results.', 'cpschool' ),
					'panel'       => 'content_area',
				)
			);

				// SETTING - Search Style
				Kirki::add_field(
					'cpschool',
					array(
						'type'      => 'select',
						'settings'  => 'search_results_style',
						'label'     => __( 'Search Result Style', 'cpschool' ),
						'section'   => 'search',
						'default'   => '',
						'transport' => 'refresh',
						'choices'   => array(
	...
	function cpschool_reusable_block_integration( $json, $settings ) {
		...
		if ( in_array( $settings['default']->id, $settings_with_blocks ) ) {
			...
			if ( ! $reusable_blocks_choices ) {
				$reusable_blocks_choices = array( 0 => __( 'Disable', 'cpschool' ) ) + Kirki_Helper::get_posts(
					array(
						'post_type'   => 'wp_block',
						'numberposts' => 100,
					)
